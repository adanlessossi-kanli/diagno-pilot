...
3. WHEN the LLM answer contains fewer than 3 parseable diagnoses, THE DiagnosticParser SHALL return a list of 3 placeholder `DifferentialDiagnosis` objects with `probability=0.0` and `icd_code=None`.
...
6. WHEN the LLM answer contains no parseable JSON array, THE DiagnosticParser SHALL log a warning and return the 3-placeholder fallback list.
7. THE DiagnosticParser SHALL contain no I/O operations, no HTTP calls, and no database access.
...
5. THE DiagnosticOrchestrator SHALL accept `rag_service`, `prompt_builder`, and `diagnostic_parser` as constructor parameters to enable dependency injection.
...
1. WHEN the existing `DiagnosticService` import path is used, THE System SHALL resolve to `DiagnosticOrchestrator` without raising an `ImportError`.
2. THE DiagnosticOrchestrator SHALL accept the same constructor arguments as the current `DiagnosticService` (`rag_service`).
...
6. WHEN a Zod schema is updated, THE corresponding TypeScript interface SHALL be derived from it (schema-first) to prevent drift between runtime and compile-time types.
...
2. IF the response body fails Zod validation, THEN THE packages/api-client SHALL throw a typed error containing the `ZodError` details.
...
2. THE Zod schemas SHALL accept both camelCase field names (TypeScript convention) and snake_case field names (Python/API convention) for fields that differ between the two conventions, or the API client SHALL normalize field names before validation.
